...
use o3whisburn_server::{routes, upload::attachment_filename, AppState};
use o3whisburn_models::ModelManager;
...
fn test_state() -> AppState {
    AppState {
        ...
        gpu: o3whisburn_server::gpu::GpuInfo {
            name: "test-gpu".into(),
            vram_mb: Some(8192),
            source: "test".into(),
        },
...
fn multipart_audio_request(uri: &str, field: &str, filename: &str, bytes: &[u8]) -> Request<Body> {
    let boundary = "----o3whisburn-test-boundary";
...
async fn health_endpoint_returns_ok() {
    ...
    assert_eq!(json["service"], "o3whisburn");
}
...
async fn root_endpoint_serves_web_ui() {
    ...
    assert!(html.contains("o3whisburn"));
    assert!(!html.contains("o2whisburn"));
...
async fn ensure_model_endpoint_returns_path_when_ready() {
    let app = routes::router().with_state(test_state());
    if !o3whisburn_models::paths::is_model_ready("tiny_en") {
        return;
    }
...
fn attachment_filename_sanitizes_stem() {
    ...
    assert_eq!(attachment_filename("", "txt"), "o3whisburn.txt");
}