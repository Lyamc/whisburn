...
use o3whisburn_core::{SpeechTask, TaskOptions, TranscriptResult, O3WhisburnError, O3WhisburnResult};
use o3whisburn_engine::model::registry::{find_model, ModelCategory};
use o3whisburn_engine::runtime::InferenceRuntime;
...
impl ModelManager {
    ...
    pub async fn ensure_model(&self, name: &str) -> O3WhisburnResult<PathBuf> {
        ...
        download_model(name, &self.download_options)
            .map_err(|e| O3WhisburnError::Model(e.to_string()))
    }
    ...
    pub async fn warm_model(&self, name: &str) -> O3WhisburnResult<()> {
    ...
    pub async fn process_waveform(
        ...
        sample_rate: usize,
        options: &TaskOptions,
    ) -> O3WhisburnResult<TranscriptResult> {
    ...
    pub fn ensure_model_sync(&self, name: &str) -> O3WhisburnResult<PathBuf> {
        if is_model_ready(name) {
            return Ok(model_dir(name));
        }
    ...
    pub fn process_waveform_sync(
        ...
        if !is_model_ready(model_name) {
            return Err(O3WhisburnError::Model(format!(
    ...
    fn validate_task(&self, model_name: &str, options: &TaskOptions) -> O3WhisburnResult<()> {
        let info = find_model(model_name)
            .ok_or_else(|| O3WhisburnError::Model(format!("unknown model: {model_name}")))?;
