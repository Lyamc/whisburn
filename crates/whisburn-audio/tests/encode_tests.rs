use whisburn_audio::{encode_result, encode_result_with_options, format_srt, group_segments_into_sentences};
use whisburn_core::{OutputFormat, TranscriptResult, TranscriptSegment};
...
/// Run with: cargo test -p whisburn-audio --test encode_tests verify_model_output_matrix -- --nocapture
#[test]
...
fn parakeet_srt_output_works() {
    ...
    // Plain SRT (the common case: whisburn transcribe --model parakeet... --format srt)
