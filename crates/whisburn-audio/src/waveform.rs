...
impl Waveform {
    ...
    pub fn ensure_sample_rate(self, target: usize) -> Result<Self, WhisburnError> {
    ...
    pub fn slice(&self, start_secs: f64, end_secs: f64) -> Result<Self, WhisburnError> {
        ...
        if start >= end {
            return Err(WhisburnError::Audio("invalid slice range".into()));
        }
        Ok(Self {
