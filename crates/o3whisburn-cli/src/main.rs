...
use o3whisburn_core::{load_settings, group_segments_into_sentences, OutputFormat, PreloadModels, SpeechTask, TaskOptions};
use o3whisburn_models::{download_model, DownloadOptions, ModelManager};
use o3whisburn_server::ServerConfig;
...
#[derive(Parser, Debug)]
#[command(name = "o3whisburn", about = "o3whisburn: Burn-based speech processing server and CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Serve {
        #[arg(long, env = "O3WHISBURN_PORT")]
        port: Option<u16>,
        #[arg(long, env = "O3WHISBURN_DEFAULT_MODEL")]
        model: Option<String>,
        #[arg(long, env = "O3WHISBURN_DEVICE")]
        device: Option<String>,
        #[arg(long, env = "HF_TOKEN")]
        hf_token: Option<String>,
        #[arg(short, long, env = "O3WHISBURN_VERBOSE")]
        verbose: bool,
        #[arg(long, env = "O3WHISBURN_DEBUG")]
        debug: bool,
        ...
        #[arg(long = "preload-models", value_delimiter = ',', num_args = 0.., env = "O3WHISBURN_PRELOAD_MODELS")]
    ...
    Transcribe {
        #[arg(short, long)]
        input: String,
        #[arg(short, long, default_value = "tiny_en", env = "O3WHISBURN_DEFAULT_MODEL")]
        model: String,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Serve {
            ...
            o3whisburn_server::run_server(config).await?;
        }
        Commands::App => {
            o3whisburn_app::run()?;
        }
        Commands::Models { action } => match action {
            ...
            ModelsCommand::List => {
                for m in o3whisburn_engine::model::registry::MODEL_REGISTRY {
                    println!(
        ...
        Commands::Transcribe {
            ...
            let audio_dur = {
                ...
                o3whisburn_audio::probe_bytes_duration_secs(&b, ext).ok().flatten()
            };
            ...
            let waveform = o3whisburn_audio::decode_to_mono_pcm(std::path::Path::new(&input))?;
            ...
            let options = TaskOptions {
                task: speech_task,
                language: o3whisburn_core::LanguageCode::new(language),
                include_timestamps,
                orchestrate,
            ...
            let encoded = o3whisburn_audio::encode_result_with_options(&result, output_format, include_timestamps, sentences)?;
        ...
        Commands::Verify {
            ...
            let model_list: Vec<String> = if models.eq_ignore_ascii_case("burn-ready") {
                o3whisburn_engine::model::registry::MODEL_REGISTRY
                    .iter()
            ...
            let waveform = o3whisburn_audio::decode_to_mono_pcm(std::path::Path::new(&sample))?;
                    ...
                    for &do_diar in &[false, diarize] {
                        for &do_sent in &[false, sentences] {
                            ...
                            let opts = TaskOptions {
                                task,
                                language: o3whisburn_core::LanguageCode::new("en".to_string()),
                            ...
                            let bytes = o3whisburn_audio::encode_result_with_options(&res, of, true, do_sent)?;
                            let tag = format!(
                                "{m}__{}__{}{}{}",
                                fmt,
