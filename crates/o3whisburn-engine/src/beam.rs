    ...
    G: Fn(&[T]) -> bool + Clone,
{
    ...
    for _ in 0..max_depth {
        let best_finished = beams
            .iter()
            .filter(|beam| is_finished(&beam.seq))
            .max_by(|a, b| {
                a.log_prob
                    .partial_cmp(&b.log_prob)
                    .unwrap_or(std::cmp::Ordering::Equal)
            });
        let best_unfinished = beams
            .iter()
            .filter(|beam| !is_finished(&beam.seq))
            .max_by(|a, b| {
                a.log_prob
        ...
        let should_stop = match (best_finished, best_unfinished) {
            (Some(_), None) => true,
            (Some(finished), Some(unfinished)) => {
                finished.log_prob >= unfinished.log_prob
            }
            _ => false,
        };
        if should_stop {
